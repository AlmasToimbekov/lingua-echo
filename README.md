...
- Тексты шаблонов и сгенерированные голоса (EN + RU) теперь сохраняются между обновлениями страницы благодаря localStorage + IndexedDB (аудио хранится как реальные байты, а не временные blob: URL). Никаких аккаунтов в текущей версии.
...
- Полноценный backend + Google OAuth (родительский аккаунт + лёгкие профили детей)
- Хранение шаблонов и аудио в облачном хранилище (чтобы дети могли заходить с любого устройства)
- Возможность принести свои ключи Gemini + ElevenLabs (или proxy на сервере)
- См. подробный план в `.grok/sessions/.../plan.md` (секция Phase 2)
...
Сгенерированные голоса теперь переживают обновление страницы благодаря хранению аудио в IndexedDB.
